...
int GisCommit(LOVENIX gps_info)
{
    int sockfd,index;
    char url[1024] = {0};
    char rBuf[2048] = {0};
    ...
    snprintf(url,sizeof(url),REQ,gps_info.lat,gps_info.lon,gps_info.time,1);
    if(GetIp("www.tgj168.com",ip) == 0)
        return 0;
//    printf("LBS[%d]: %s\n",strlen(url),url);
    speed = atod(gps_info.speed,strlen(gps_info.speed));
    sprintf(temp,"%.1f",speed*1.852);
    sprintf(sql,"insert into pre_saas_thing_record (thingsn,thinglon,thinglat,recodedate,recodetime,thingdirection,thingspeed) values ('%s','%s','%s','%s','%s','%s','%s')",gps_info.id,gps_info.lat,gps_info.lon,gps_info.date,gps_info.time,gps_info.dir,temp);
    DbInsert(env,sql);
