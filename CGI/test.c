...
int parseJson(char * pMsg)
{
    ...
    cJSON * pSub = cJSON_GetObjectItem(pJson, "gateId");	// get string from json
    if(NULL == pSub)
    {
        printf("Not found gateId!\n");
        return 0;		// get object named "gateId" faild
    }
    else
    {
      ;//  printf("gateId : %s\n", pSub->valuestring);
    }
    strncat(msg,pSub->valuestring,16);
    pSub = cJSON_GetObjectItem(pJson, "deviceId");
    if(NULL == pSub)
    {
        printf("Not found deviceId!\n");
        return 0;		// get deviceId from json faild
    }
    else
       ;// printf("deviceId : %s\n", pSub->valuestring); //valueint
    strncat(msg,pSub->valuestring,8);
    /*
